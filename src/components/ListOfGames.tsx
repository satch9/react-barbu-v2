...
const ListOfGames: React.FC<ListOfGamesProps> = ({ socketState, roomsState, setGameStarted }) => {
    ...
    const handleJoinGame = useCallback((roomIdChooseGame: string) => {
        openModal1()
    ...
    const handleSubmit = useCallback((e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (socketState.socket && pseudo !== "") {
            socketState.socket.emit('create_game', { uid: socketState.uid, socketId: socketState.socket.id, pseudo: pseudo })
        }
        closeModal()
    }, [socketState.socket, pseudo, socketState.uid]);


    const handleSubmit1 = useCallback((e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        setPseudo(pseudo)
        if (socketState.socket && pseudo !== "") {
            socketState.socket.emit('join_game', { roomId: roomIdJoinedGame, uid: socketState.uid, socketId: socketState.socket.id, pseudo: pseudo })
        }
        closeModal1()
    }, [socketState.socket, pseudo, socketState.uid, roomIdJoinedGame]);

    const handleStartGame = useCallback((roomIdStartGame: string) => {
    ...
    const memoizedComponent = useMemo(() => (
        <div className='listofgames'>
                        ...
                        roomsState.rooms.map((room, index) => (
                            <tr key={index}>
                                ...
                                <td className="btn-actions">
                                    ...
                                    {
                                        (roomsState.rooms.length !== 0 && room.players.length !== 4 && !joinedRoom.includes(room.roomId) && room.players[0].socketId !== socketState.socket?.id) && (
                                            <button
                                                className="btn-join"
                                    ...
                                    {
                                        ((roomsState.rooms.length !== 0 && room.players.length === 4 && (
            ...
            >
                <div className="modal">
                    ...
                    <form onSubmit={handleSubmit}>
                        <input type="text" name="name" id="name" required placeholder='pseudo' value={pseudo}
                            onChange={(e) => setPseudo(e.target.value)} />

                        <button className="btn btn-modal" type="submit" >Choisir</button>
                    </form>
            ...
            >
                <div className="modal">
                    <div className='modal_close_container'>
                        <button className="modal__close" onClick={closeModal1}>X</button>
                    </div>
    ...
    ), [roomsState.rooms, handleCreateGame, modalIsOpen, handleSubmit, pseudo, modalIsOpen1, handleSubmit1, joinedRoom, socketState.socket?.id, handleJoinGame, handleStartGame]);
