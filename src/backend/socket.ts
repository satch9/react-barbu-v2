...
export class ServerSocket {
  ...
  constructor(httpServer: HttpServer) {
    ...
    this.io = new ServerSocketIo(httpServer, {
      ...
      cookie: false,
      cors: {
        origin: "https://9r5lxf-5173.csb.app",
        methods: ["GET", "POST"],
        credentials: true,
      },
    });
  ...
  StartListeners(socket: Socket) {
    ...
    socket.on("start_game", ({ roomId }) => {
      ...
      this.roomsState.rooms.forEach((room) => {
        ...
        const matchingGamePlayers = this.gameState.players.filter(
          (gamePlayer) =>
            room.players.some(
              (roomPlayer) => roomPlayer.socketId === gamePlayer.socketId
            )
        );

        matchingGamePlayers.forEach((gamePlayer) => {
          const matchingRoomPlayer = room.players.find(
            (roomPlayer) => roomPlayer.socketId === gamePlayer.socketId
          );
      ...
      this.gameState.currentPlayer =
        this.gameState.players[this.randomCurrentPlayer() - 1];
    ...
    socket.on("contract_choice", (index, playerContractChoose) => {
      console.log(
        `Contract choice received from ${playerContractChoose.name} => contract: ${index}`
      );
    ...
    socket.on("gobackgame", ({ roomIdGoBackGame }) => {
      ...
      this.roomsState.rooms.forEach((room) => {
        if (room.roomId === roomIdGoBackGame) {
          this.io.emit("gameState", this.gameState);
          this.io.emit("roomsState", this.roomsState);
        }
      });
    ...
    socket.on("card_played", ({ cardClicked, playerClickedCards }) => {
      ...
      if (playerIndex !== -1) {
        ...
        const cardIndex = newPlayer.startedHand.findIndex((card) => {
          ...
          console.log("cardClicked", cardClicked);
          return (
            card.suit === cardClicked.suit && card.value === cardClicked.value
          );
        });
      ...
      this.roomsState.rooms.forEach((room) => {
        ...
        if (roomPlayerIndex !== -1) {
          ...
          const roomCardIndex = newRoomPlayer.startedHand.findIndex(
            (card) =>
              card.suit === cardClicked.suit && card.value === cardClicked.value
          );
          if (roomCardIndex !== -1) {
            newRoomPlayer.startedHand.splice(roomCardIndex, 1);
            newRoomPlayer.myHandsDuringGame = [
              ...newRoomPlayer.myHandsDuringGame,
              cardClicked,
            ];
            room.players.forEach((player) => {
