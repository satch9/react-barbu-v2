...
const App = () => {
  ...
  const [GameStateReducer, gameDispatch] = useReducer(
    gameReducers,
    initialState
  );
  const [RoomsStateReducer, roomsDispatch] = useReducer(
    roomReducers,
    initialRoomsState
  );
  ...
  const socket = useSocket("https://9r5lxf-4003.csb.app", {
    ...
    <>
      <div className="app">
        ...
        <div className="global">
          <span>
            Etat du jeu :<pre>{JSON.stringify(GameStateReducer, null, 1)}</pre>
          </span>
          <span>
            Etat des rooms :
